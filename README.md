...
<p align="center">
    <img src="https://img.shields.io/static/v1.svg?label=version&message=4.0.1&color=yellow">
</p>

# Notes (read first)

This documentation is deprecated (current version is 4.0.1, white this was written for < 3.6.0)

Notable differences, while it's in process of redacting:

1. `php artisan win5x:subscribe` is removed. Updated WS implementation will come soon, as previous one was unstable.
2. Scheduler is now required to be configured, you can't skip this step.
3. You can now install official `laravel-echo-server` instead of fork.
4. **Sport betting code is obfuscated for the time being.** However as a compromise you can use them without any monthly fees while we improve them.
5. License checks are removed. Some of our clients were mad about two obfuscated little files out of all of them, so you can now embrace 100% of casino source code.
6. /nodes/ folder has normalized names now.

```
About slightly delayed support response messages: we can't answer everyone immediately.
If you have "Advanced package" (https://playin.team/), we will prioritize support for you.
```

```
4.0 is, probably, the latest major update of Win5X available for everyone to purchase.

We will continue supporting it for existing websites.

What's next? Upgraded sport betting & casino, written on Spring and Vue.js, with the support of community plugins and website/admin panel constructor.
The easiest, most customizable and more cheaper way to manage casino website. Available near the end of 2021 @ playin.team.
```

Again, documentation below may be outdated:
...
- If NPM repository is down, install it from [https://github.com/Win5X-Team/laravel-echo-server](https://github.com/Rivemont-Team/laravel-echo-server)
...
#### Skip this step if you will run nodes and Rivemont on same server.
