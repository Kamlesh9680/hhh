...
abstract class Currency {
    ...
    public function getOptions(): array {
        return array_merge($this->options(), [
            ...
            new class extends WalletOption {
                public function id() {
                    return 'invest_all_time_fee';
                }
                public function name(): string {
                    return 'Investment fee from website profit on all-time high (default: 60) (%)';
                }
            },
            new class extends WalletOption {
                public function id() {
                ...
                public function name(): string {
                    return 'Investment withdraw fee (%)';
                }
            },
            new class extends WalletOption {
                function id() {
    ...
    public static function toCurrencyArray(array $array) {
        $currency = [];
        foreach($array as $c) $currency = array_merge($currency, [
            $c->id() => [
                'id' => $c->id(),
                'walletId' => $c->walletId(),
                'name' => $c->name(),
            ...
            'vipClosest' => 'rub',//auth('sanctum')->guest() ? Currency::all()[0]->id() : auth('sanctum')->user()->closestVipCurrency()->walletId(),
            'vipClosestWager' => 0//auth('sanctum')->guest() ? 0 : DB::table('games')->where('user', auth('sanctum')->user()->_id)->where('currency', auth('sanctum')->user()->closestVipCurrency()->id())->where('demo', '!=', true)->where('status', '!=', 'in-progress')->where('status', '!=', 'cancelled')->sum('wager')
        ]);
