<template>
    <div :class="'betSlip ' + (betSlip ? 'visible' : 'hidden')">
        <div class="betSlipHeader">
            <div :class="'tab ' + (tab === 'betSlip' ? 'active' : '')" @click="tab = 'betSlip'">{{ $t('sport.bet_slip') }}</div>
            <div :class="'tab ' + (tab === 'myBets' ? 'active' : '')" @click="tab = 'myBets'" v-if="!isGuest">{{ $t('sport.my_bets') }}</div>
        ...
        <template v-if="tab === 'betSlip'">
            ...
            <overlay-scrollbars ref="betSlipScrollbar" :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }" class="bets" v-if="bets.length > 0">
                <div :class="'bet ' + (bet.disabled || !bet.runner.open || !bet.game.open || !bet.market.open ? 'disabled' : '')" v-for="bet in bets">
                    <div class="betHeader">
                        <div class="game">{{ bet.game.name }}</div>
                        <i class="fal fa-times" @click="removeBet(bet)"></i>
                    </div>
                    <div class="betContainer">
                        <div class="oddsChange" v-if="bet.game.open && bet.market.open && bet.runner.open && !bet.disabled && bet.verifyOddsChange">
                        ...
                        <div class="controls">
                            ...
                            <div class="control" v-if="isSingle">
                                <input v-model="bet.value" v-money="money">
            ...
            <div class="betSlipManagement">
                <div class="multiBet" v-if="!isSingle">
                    <input v-model="multiBet" v-money="money">
                    <icon :icon="currencies[currency].icon" :style="{ color: currencies[currency].style }"></icon>
                </div>
                <div class="value">
                    <div class="name">
                    ...
                    <div class="val">
                        ...
                        <icon v-if="isSingle" :icon="currencies[currency].icon" :style="{ color: currencies[currency].style }"></icon>
                    </div>
                </div>
                <div class="value">
                    <div class="name">
                ...
                <div :class="'btn btn-primary btn-block ' + (hasUnverifiedOddsChanges() || betting || bets.length === 0 ? 'disabled' : '')" :disabled="hasUnverifiedOddsChanges() || betting || bets.length === 0" @click="isGuest ? openAuthModal('auth') : bet()">{{ $t(isGuest ? 'sport.register_to_bet' : 'sport.bet') }}</div>
            </div>
        </template>
        <template v-else-if="!isGuest">
            ...
            <div v-else-if="userGames.length === 0" class="bets noFooter empty">
                <icon icon="scroll"></icon>
                <div class="title">{{ $t('sport.no_bets_title') }}</div>
            </div>
            <overlay-scrollbars ref="betSlipScrollbar" :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }" class="bets noFooter" v-if="userGames && userGames.length > 0">
                <div class="bet" v-for="bet in userGames">
                    <div class="betHeader">
                    ...
                    <div class="betContainer">
                        <div class="controls">
                            ...
                            <icon :icon="currencies[bet.currency].icon" :style="{ color: currencies[bet.currency].style }"></icon>
                        </div>
                        <div class="info">
                            <div class="payout">{{ bet.odds }}</div>
                        </div>
                    </div>
                    <div class="gameOutcome" v-if="betsTab === 'finished'">
    ...
    export default {
        ...
        created() {
            Bus.$on('betSlip:add', (data) => {
                ...
                setTimeout(() => this.$refs.betSlipScrollbar._osInstance.scroll({ y: '100%' }), 50);
            });
        ...
        methods: {
            ...
            bet() {
                ...
                axios.post('/api/sport/bet', {
                    ...
                    this.$toast.error(this.$i18n.t('sport.error'));
                    this.betting = false;
                    console.log(e);
                });
            },
            ...
            total() {
                let total = 0;
                this.bets.forEach((bet) => {
                    total += parseFloat(this.isSingle ? bet.value : bet.runner.price);
                });
                return total;
            },
            estPayout() {
...
<style lang="scss">
    ...
    @include media-breakpoint-down(md) {
        .betSlip {
            ...
            .bets {
                height: calc(100% - #{$management-height} - 73px - 102px) !important;
            }
        }
    }
</style>
