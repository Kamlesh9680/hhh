            ...
            <div class="header">
                ...
                <template v-else>
                    <div class="live_tabs">
                        <div class="tabs">
                            <div v-if="!isGuest" @click="$store.dispatch('setLiveChannel', 'mine')" :class="`tab ${liveChannel === 'mine' ? 'active' : ''}`">{{ $t('general.bets.mine') }}</div>
                            <div @click="$store.dispatch('setLiveChannel', 'all')" :class="`tab ${liveChannel !== 'mine' ? 'active' : ''}`">{{ $t('general.bets.all') }}</div>
                        </div>
                        <select @change="$store.dispatch('setLiveFeedEntryCount', parseFloat(liveFeedEntriesWrap))" v-model="liveFeedEntriesWrap">
                            <option value="10" :selected="liveFeedEntries === 10">10</option>
                            <option value="25" :selected="liveFeedEntries === 25">25</option>
                                ...
                                <th>
                                    <div class="gameIcon">
                                        <router-link :to="`/sport/game/`" tag="div" class="icon d-none d-md-inline-block">
                                            <icon :icon="game.game.icon"></icon>
                                        </router-link>
                                        <div class="name">
                                            <div><router-link :to="`/sport/category/` + game.game.category">{{ game.game.game }}</router-link></div>
                                            <router-link :to="`/sport/category/` + game.game.category">{{ game.game.category }}</router-link>
                                        </div>
                                    </div>
                                </th>
                                <th>
