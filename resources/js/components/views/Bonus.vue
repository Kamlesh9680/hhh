<template>
    <div class="container-fluid">
        ...
        <div class="promotions">
            ...
            <div class="promotion" :style="{ 'background-image': `url('/img/misc/promotions/vip.jpg')`, color: 'black' }" @click="vip()">
            ...
            <div class="promotion" :style="{ 'background-image': `url('/img/misc/promotions/rain.jpg')`, color: 'white' }" @click="rain()">
            ...
            <div class="promotion" :style="{ 'background-image': `url('/img/misc/promotions/affiliates.jpg')`, color: 'white' }" @click="affiliates()">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['user', 'isGuest', 'locale', 'user'])
        },
        methods: {
            faucet() {
                if(this.isGuest) AuthModal.methods.open('auth');
                else {
                    if(!this.user.user.vk && this.locale === 'ru') {
                        NotificationModal.methods.open(`${this.$i18n.t('bonus.no_vk')}<br><br><btn class="btn btn-block btn-primary" onclick="window.location.href = '/profile/${this.user.user._id}#settings'">${ this.$i18n.t('bonus.link') }</btn>`);
                    } else {
                        if(this.locale === 'ru') {
                            axios.post('/api/promocode/verifyVkGroup').then(({ data }) => {
                                if(data.subscribed) {
                                    this.sidebarComponent = 'promotion-faucet';
                                    this.toggleSidebar = true;
                                } else NotificationModal.methods.open(`${this.$i18n.t('bonus.join_group')}<br><br><button class="btn btn-block btn-primary" onclick="window.open('https://vk.com/rivemont', '_blank')">${ this.$i18n.t('bonus.join') }</button>`)
                            })
                        } else {
                            this.sidebarComponent = 'promotion-faucet';
                            this.toggleSidebar = true;
